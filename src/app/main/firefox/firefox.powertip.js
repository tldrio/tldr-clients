...
(function($) {
  ...
  function TooltipController(options) {
    ...
    if (tipElement.length === 0) {
      tipElement = $('<div></div>', { id: options.popupId }).addClass('firefox');
    ...
    function showTip(element) {
      ...
      self.port.once('POSITION_POPOVER', function() {
        if (!options.followMouse) {
          positionTipOnElement(element);
          session.isFixedPopOpen = true;
        } else {
        ...
        // fadein
        tipElement.animate( { opacity: 1 }, function() {
          // start desync polling
          if (!session.desyncTimeout) {
            session.desyncTimeout = setInterval(closeDesyncedTip, 500);
          }
    ...
    function hideTip(element) {
      if (!element.data('stayhere')) {
        ...
        // fade out
        tipElement.animate({ opacity: 0}, function() {
          session.isClosing = false;
          session.isFixedPopOpen = false;
          //tipElement.removeClass();
          //chrome.extension.sendMessage({ action: 'REMOVE_POPOVER_TIP' });
          self.port.emit('REMOVE_POPOVER_TIP');
    ...
    function positionTipOnElement(element) {
      ...
      DEVMODE && unsafeWindow.console.log('finalPlacement', finalPlacement);
      //chrome.extension.sendMessage({ action: 'SEND_POPOVER_TIP', placement: finalPlacement });
      self.port.emit('SEND_POPOVER_TIP', { placement: finalPlacement });
