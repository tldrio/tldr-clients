...
(function($) {
  ...
  function TooltipController(options) {
    ...
    if (tipElement.length === 0) {
      ...
      iframeElement = $('<iframe></iframe>', { id: options.iframeId
                                             , src: options.iframeSrc
                                             , scrolling: 'no'
                                             , frameborder: '0'
                                             });
      tipElement.append(iframeElement);
    ...
    function showTip(element) {
      ...
      //DEVMODE && console.log('show tip function called');

      // set popup position callback
      function positionPopoverCallback (message, sender, callback) {
        if (message.action === 'POSITION_POPOVER') {
          //DEVMODE && console.log('received message to position popover');
          if (!options.followMouse) {
          ...
          chrome.extension.onMessage.removeListener(positionPopoverCallback);
        }
      }

      chrome.extension.onMessage.addListener(positionPopoverCallback);
    ...
    function hideTip(element) {
      if (!element.data('stayhere')) {
        ...
        tipElement.fadeOut(options.fadeOutTime, function() {
          ...
          chrome.extension.sendMessage({ action: 'REMOVE_POPOVER_TIP' });
    ...
    function positionTipOnElement(element) {
      //DEVMODE && console.log('positionTipOnElement called');
      ...
      if (options.smartPlacement) {
        ...
        $.each(priorityList, function(idx, pos) {
          ...
          //DEVMODE && console.log('popover height', tipHeight);
          ...
          // find collisions
          //console.log(placementCoords, tipWidth, tipHeight);
          ...
          var totalCollisions = collisions[0]+collisions[1]+collisions[2]+collisions[3];
          //console.log(pos, totalCollisions);
          ...
          if (totalCollisions === 0) {
            ...
            if (totalCollisions < smallestCollisions) {
              smallestCollisions = totalCollisions;
              finalPlacement = pos;
              finalCoords = placementCoords;
            }
          }
      ...
      //DEVMODE && console.log('finalPlacement', finalPlacement);
      chrome.extension.sendMessage({ action: 'SEND_POPOVER_TIP', placement: finalPlacement });
      //tipElement.addClass(finalPlacement);

      // position the tooltip
      setTipPosition(finalCoords.x, finalCoords.y);
    }
    ...
    function computePlacementCoords(element, placement, popWidth, popHeight) {
      ...
      var borderTopOffset = (parseInt($('body').css('border-top-width'),10) + parseInt($('html').css('border-top-width'), 10)) || 0,
        ...
        objectOffset.top = objectOffset.top + borderTopOffset;

      // calculate the appropriate x and y position in the document
      switch (placement) {
      case 'n':
        x = (objectOffset.left + (objectWidth / 2)) - (popWidth / 2);
        y = objectOffset.top - popHeight - options.offset + 10;
        break;
      case 'e':
        x = objectOffset.left + objectWidth + options.offset - 10;
        y = (objectOffset.top + (objectHeight / 2)) - (popHeight / 2);
        break;
      case 's':
        x = (objectOffset.left + (objectWidth / 2)) - (popWidth / 2);
        y = objectOffset.top + objectHeight + options.offset - 10;
        break;
      case 'w':
        x = objectOffset.left - popWidth - options.offset + 10;
        y = (objectOffset.top + (objectHeight / 2)) - (popHeight / 2);
        break;
      case 'nw':
  ...
  function isMouseOver(element) {
    var elementPosition = element.offset()
      , borderTopOffset = (parseInt($('body').css('border-top-width'),10) + parseInt($('html').css('border-top-width'), 10)) || 0
    ;
    elementPosition.top = elementPosition.top + borderTopOffset;
  ...
  function getViewportCollisions(coords, elementWidth, elementHeight) {
    var scrollLeft = $window.scrollLeft(),
      ...
      windowWidth = $window.width(),
      // necessary, seems that $(window).height() doesn't work properly in chrome extension
      // it returns the size of document body instead of viewport for some reason
      ...
      var collisionTop = Math.max(0, scrollTop - coords.y);
      var collisionBottom = Math.max(0, coords.y - scrollTop + elementHeight - windowHeight);
      var collisionLeft = Math.max(0, scrollLeft - coords.x);
      var collisionRight = Math.max(0, coords.x + elementWidth - windowWidth);
      collisions = [collisionTop, collisionBottom, collisionLeft, collisionRight];

    //if (coords.y < scrollTop) {
      //collisions.push('top');
    //}
    //if (coords.y + elementHeight > windowHeight) {
      //collisions.push('bottom');
    //}
    //if (coords.x < scrollLeft) {
      //collisions.push('left');
    //}
    //if (coords.x + elementWidth > windowWidth) {
